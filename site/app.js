...
function renderNationGrid(doc) {
  ...
  for (const [code, entry] of entries) {
    ...
    card.innerHTML = `
      ...
      <div class="pair">${top ? `${top.player_a_display || top.player_a_name} + ${top.player_b_display || top.player_b_name}` : "no in-squad pairs"}
...
function renderLeaderboard(doc) {
  ...
  for (let i = 0; i < doc.leaderboard.length; i++) {
    ...
    tr.innerHTML = `<td>${i + 1}</td>
      <td>${flag}${p.player_a_display || p.player_a_name} + ${p.player_b_display || p.player_b_name}</td>
